...
def candidate_to_dict(candidate: Candidate) -> dict:
    return {
        "helper_name": candidate.helper_name,
        "return_type": candidate.return_type,
        "classname": candidate.classname,
        "interface": candidate.interface,
        "methodname": candidate.methodname,
        "flat_name": candidate.flat_name,
        "accessor": candidate.accessor,
        "sdk_return_type": candidate.sdk_return_type,
        "inputs": [
            {"type": param.type, "name": param.name}
            for param in candidate.inputs
        ],
        "outputs": [
            {
                "kind": output.kind,
        ...
        "call_args": list(candidate.call_args),
...
def candidate_from_dict(data: dict) -> Candidate:
    return Candidate(
        ...
        sdk_return_type=data["sdk_return_type"],
        inputs=tuple(Param(param["type"], param["name"]) for param in data.get("inputs", [])),
        outputs=tuple(
            Output(
                output["kind"],
                output["type"],
                output["name"],
                output.get("size_name"),
            )
            for output in data.get("outputs", [])
        ),
        call_args=tuple(data.get("call_args", [])),
        reason=data["reason"],
    )
...
def promoted_candidate_dicts(api: dict, flat_identifiers: set[str]) -> list[dict]:
    return [candidate_to_dict(candidate) for candidate in promoted_candidates(api, flat_identifiers)]


def output_helper_functions(candidates: list[Candidate]) -> list[tuple]:
    return [
        (
...
def output_helper_functions_from_model(model: dict) -> list[tuple]:
    return output_helper_functions(
        [candidate_from_dict(candidate) for candidate in model.get("output_helpers", [])]
    )


def generated_output_helper_functions(api: dict, flat_identifiers: set[str]) -> list[tuple]:
    return output_helper_functions(promoted_candidates(api, flat_identifiers))
...
def output_helper_declarations_from_model(model: dict) -> str:
    return "\n".join(
        declaration(candidate_from_dict(candidate))
        for candidate in model.get("output_helpers", [])
    )
...
def output_helper_definitions_from_model(model: dict) -> str:
    return "\n\n".join(
        definition(candidate_from_dict(candidate))
        for candidate in model.get("output_helpers", [])
    )
...
def report(candidates: list[Candidate], promoted: set[str] | None = None) -> str:
    ...
    lines = [
        ...
        "Promoted candidates are recorded by `tools/generate_model.py` and rendered by `tools/generate_core.py`; the rest are review-only.",
        "",
        "## Summary",
        "",
    ]
